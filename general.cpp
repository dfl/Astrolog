...
real RSgn(real r) { return r == 0.0 ? 0.0 : RSgn2(r); }
...
CONST int rgnHue2[cSign + 1] = {0,   15,  30,  45,  60,  75, 120,
...
KV KvHue2(real deg) {
  ...
  while (deg < 0.0)
    deg += rHueMax;
  sig = SFromZ(deg) - 1;
  deg = (real)rgnHue2[sig] + (deg - (real)(sig * 30)) / 30.0 *
                                 (real)(rgnHue2[sig + 1] - rgnHue2[sig]);
  return KvHue(deg);
}
...
real SphDistance(real lon1, real lat1, real lon2, real lat2) {
  ...
  dLon = RAbs(lon1 - lon2);
  r = RAcosD(RSinD(lat1) * RSinD(lat2) +
             RCosD(lat1) * RCosD(lat2) * RCosD(dLon));
  return r;
}
...
real RObjDiam(int obj) {
  ...
  if (FBetween(obj, oJup, oPlu) &&
      (!ignore[obj - oJup + cobLo]
#ifdef SWISS
...
flag FErrorSubswitch(CONST char *szOpt, char chSub, flag f) {
  ...
  if (!f)
    return fFalse;
  sprintf(sz, "Unknown subswitch '%c%s%c' to command switch '%c%s'", chSwitch,
          szOpt, chSub, chSwitch, szOpt);
  PrintError(sz);
  return fTrue;
}
...
void PrintZodiac(real deg) {
  if (us.fRound) {
    if (us.nDegForm == dfZod)
      deg = Mod(deg + VSeconds(rRound / 60.0, rRound / 3600.0,
                               rRound / 3600.0 / 1000.0));
    else if (us.nDegForm == dfHM)
      deg = Mod(deg + VSeconds(rRound / 4.0, rRound / 4.0 / 60.0,
                               rRound / 4.0 / 60.0 / 1000.0));
  }
  ...
  default:
    ...
    if (us.fSeconds) {
      if (!us.fSecond1K) {
        ...
        d = (int)((deg - (real)sign * (rDegMax / 27.0)) * 40000.0 /
                  (rDegMax / 27.0));
...
char *SzDate(int mon, int day, int yea, int nFormat) {
  ...
  if (us.fEuroDate) {
    switch (nFormat) {
    case 2:
      sprintf(szDat, "%2d %.3s %4d", day, szMonth[mon], yea);
      break;
    case 1:
      sprintf(szDat, "%d %s %d", day, szMonth[mon], yea);
      break;
    case -1:
      sprintf(szDat, "%2d-%2d-%2d", day, mon, NAbs(yea) % 100);
      break;
    default:
    ...
    switch (nFormat) {
    case 3:
      sprintf(szDat, "%.3s %2d, %d", szMonth[mon], day, yea);
      break;
    case 2:
      sprintf(szDat, "%.3s %2d %4d", szMonth[mon], day, yea);
      break;
    case 1:
      sprintf(szDat, "%s %d, %d", szMonth[mon], day, yea);
      break;
    case -1:
      sprintf(szDat, "%2d/%2d/%2d", mon, day, NAbs(yea) % 100);
      break;
    default:
...
char *SzTimeR(int hr, int min, int sec, real rSec) {
  ...
  if (us.fAnsiChar == 4) {
    ...
    sprintf(szTim, "%02d:%02d:%02d %cM", Mod12(hr), min, sec,
            hr < 12 ? 'A' : 'P');
    return szTim;
  }
  if (us.fEuroTime) {
    ...
    if (sec < 0)
      ...
      sprintf(szTim, "%2d:%02d:%02d%cm", Mod12(hr), min, sec,
              hr < 12 ? 'a' : 'p');
    else
      sprintf(szTim, "%2d:%02d:%06.3f%cm", Mod12(hr), min, rSec,
              hr < 12 ? 'a' : 'p');
  }
  return szTim;
}
...
char *SzZone(real zon) {
  ...
  if (zon == zonLMT)
    ...
    sprintf(szZon, "%d:%02d%c", (int)RAbs(zon),
            (int)(RFract(RAbs(zon)) * 60.0 + rRound / 60.0),
            zon < 0.0 ? 'E' : 'W');
  return szZon;
}
...
char *SzLocation(real lon, real lat) {
  ...
  if (us.fAnsiChar == 4) {
    ...
    if (!us.fSeconds)
      sprintf(szLoc, "%.0f%c%02d,%.0f%c%02d", RFloor(RAbs(lat)), chLat, j,
              RFloor(RAbs(lon)), chLon, i);
    else
      sprintf(szLoc, "%.0f%c%02d:%02d,%.0f%c%02d:%02d", RFloor(RAbs(lat)),
              chLat, j, j2, RFloor(RAbs(lon)), chLon, i, i2);
    return szLoc;
  }
  if (us.fAnsiChar != 3) {
    chDeg = ChDeg();
    if (us.nDegForm != df360) {
      if (!us.fSeconds)
        sprintf(szLoc, "%3.0f%c%02d%c%3.0f%c%02d%c", RFloor(RAbs(lon)), chDeg,
                i, chLon, RFloor(RAbs(lat)), chDeg, j, chLat);
      else if (!us.fSecond1K)
        sprintf(szLoc, "%3.0f%c%02d'%02d%c%3.0f%c%02d'%02d%c",
                RFloor(RAbs(lon)), chDeg, i, i2, chLon, RFloor(RAbs(lat)),
                chDeg, j, j2, chLat);
      else
        sprintf(szLoc, "%3.0f%c%02d'%02d.%03d%c%3.0f%c%02d'%02d.%03d%c",
                RFloor(RAbs(lon)), chDeg, i, i2, i3, chLon, RFloor(RAbs(lat)),
                chDeg, j, j2, j3, chLat);
    } else {
      if (!us.fSeconds)
        sprintf(szLoc, "%6.2f%c%6.2f%c", RAbs(lon), chLon, RAbs(lat), chLat);
      else
        sprintf(szLoc, "%9.5f%c%9.5f%c", RAbs(lon), chLon, RAbs(lat), chLat);
    }
  } else {
    if (us.nDegForm != df360) {
      if (!us.fSeconds)
        sprintf(szLoc, "%3.0f%c%02d%3.0f%c%02d", RFloor(RAbs(lon)), chLon, i,
                RFloor(RAbs(lat)), chLat, j);
      else if (!us.fSecond1K)
        sprintf(szLoc, "%3.0f%c%02d'%02d%3.0f%c%02d'%02d", RFloor(RAbs(lon)),
                chLon, i, i2, RFloor(RAbs(lat)), chLat, j, j2);
      else
        sprintf(szLoc, "%3.0f%c%02d'%02d.%03d%3.0f%c%02d'%02d.%03d",
                RFloor(RAbs(lon)), chLon, i, i2, i3, RFloor(RAbs(lat)), chLat,
                j, j2, j3);
    } else {
      if (!us.fSeconds)
        sprintf(szLoc, "%5.1f%c%5.1f%c", RAbs(lon), chLon, RAbs(lat), chLat);
      else
        sprintf(szLoc, "%8.4f%c%8.4f%c", RAbs(lon), chLon, RAbs(lat), chLat);
    }
  }
  return szLoc;
}
        ...
        case 0:
          fCompare = (pci1->yea > pci2->yea ||
                      (pci1->yea == pci2->yea &&
                       (pci1->mon > pci2->mon ||
                        (pci1->mon == pci2->mon &&
                         (pci1->day > pci2->day || (pci1->day == pci2->day &&
                                                    pci1->tim > pci2->tim))))));
          break;
        case 1:
...
CONST wchar rgwch437[128] = {
    0xc7,   0xfc,   0xe9,   0xe2,   0xe4,   0xe0,   0xe5,   0xe7,   0xea,
    0xeb,   0xe8,   0xef,   0xee,   0xec,   0xc4,   0xc5,   0xc9,   0xe6,
    0xc6,   0xf4,   0xf6,   0xf2,   0xfb,   0xf9,   0xff,   0xd6,   0xdc,
    0xa2,   0xa3,   0xa5,   0x20a7, 0x0192, 0xe1,   0xed,   0xf3,   0xfa,
    0xf1,   0xd1,   0xaa,   0xba,   0xbf,   0x2310, 0xac,   0xbd,   0xbc,
    0xa1,   0xab,   0xbb,   0x2591, 0x2592, 0x2593, 0x2502, 0x2524, 0x2561,
    ...
    0x250c, 0x2588, 0x2584, 0x258c, 0x2590, 0x2580, 0x03b1, 0xdf,   0x0393,
    0x03c0, 0x03a3, 0x03c3, 0xb5,   0x03c4, 0x03a6, 0x0398, 0x03a9, 0x03b4,
    0x221e, 0x03c6, 0x03b5, 0x2229, 0x2261, 0xb1,   0x2265, 0x2264, 0x2320,
    0x2321, 0xf7,   0x2248, 0xb0,   0x2219, 0xb7,   0x221a, 0x207f, 0xb2,
    0x25a0, 0xa0};
...
uchar ChLowAscii(wchar wch) {
  ...
  switch (wch) {
  case 0x00e1:
    return 'a'; // Latin Small Letter A with acute
  case 0x00c1:
    return 'A'; // Latin Capital Letter A with acute
  case 0x00e2:
    return 'a'; // Latin Small Letter A with circumflex
  case 0x00c2:
    return 'A'; // Latin Capital Letter A with circumflex
  case 0x00e0:
    return 'a'; // Latin Small Letter A with grave
  case 0x00c0:
    return 'A'; // Latin Capital Letter A with grave
  case 0x00e4:
    return 'a'; // Latin Small Letter A with umlaut
  case 0x00c4:
    return 'A'; // Latin Capital Letter A with umlaut
  case 0x00e5:
    return 'a'; // Latin Small Letter A with ring above
  case 0x00c5:
    return 'A'; // Latin Capital Letter A with ring above
  case 0x00e3:
    return 'a'; // Latin Small Letter A with tilde
  case 0x00c3:
    return 'A'; // Latin Capital Letter A with tilde
  case 0x00e6:
    return 'a'; // Latin Small Letter AE
  case 0x00c6:
    return 'A'; // Latin Capital Letter AE
  case 0x0101:
    return 'a'; // Latin Small Letter A with macron
  case 0x0100:
    return 'A'; // Latin Capital Letter A with macron
  case 0x0103:
    return 'a'; // Latin Small Letter A with breve
  case 0x0102:
    return 'A'; // Latin Capital Letter A with breve
  case 0x0105:
    return 'a'; // Latin Small Letter A with ogonek
  case 0x0104:
    return 'A'; // Latin Capital Letter A with ogonek
  case 0x00c7:
    return 'C'; // Latin Capital Letter C with cedilla
  case 0x00e7:
    return 'c'; // Latin Small Letter C with cedilla
  case 0x0106:
    return 'C'; // Latin Capital Letter C with acute
  case 0x0107:
    return 'c'; // Latin Small Letter C with acute
  case 0x0108:
    return 'C'; // Latin Capital Letter C with circumflex
  case 0x0109:
    return 'c'; // Latin Small Letter C with circumflex
  case 0x010c:
    return 'C'; // Latin Capital Letter C with caron
  case 0x010d:
    return 'c'; // Latin Small Letter C with caron
  case 0x010e:
    return 'D'; // Latin Capital Letter D with caron
  case 0x010f:
    return 'd'; // Latin Small Letter D with caron
  case 0x0110:
    return 'D'; // Latin Capital Letter D with stroke
  case 0x0111:
    return 'd'; // Latin Small Letter D with stroke
  case 0x00e9:
    return 'e'; // Latin Small Letter E with acute
  case 0x00c9:
    return 'E'; // Latin Capital Letter E with acute
  case 0x00ea:
    return 'e'; // Latin Small Letter E with circumflex
  case 0x00ca:
    return 'E'; // Latin Capital Letter E with circumflex
  case 0x00e8:
    return 'e'; // Latin Small Letter E with grave
  case 0x00c8:
    return 'E'; // Latin Capital Letter E with grave
  case 0x00eb:
    return 'e'; // Latin Small Letter E with umlaut
  case 0x00cb:
    return 'E'; // Latin Capital Letter E with umlaut
  case 0x0113:
    return 'e'; // Latin Small Letter E with macron
  case 0x0112:
    return 'E'; // Latin Capital Letter E with macron
  case 0x0115:
    return 'e'; // Latin Small Letter E with breve
  case 0x0114:
    return 'E'; // Latin Capital Letter E with breve
  case 0x0119:
    return 'e'; // Latin Small Letter E with ogonek
  case 0x0118:
    return 'E'; // Latin Capital Letter E with ogonek
  case 0x0117:
    return 'e'; // Latin Small Letter E with dot above
  case 0x0116:
    return 'E'; // Latin Capital Letter E with dot above
  case 0x011b:
    return 'e'; // Latin Small Letter E with caron
  case 0x011a:
    return 'E'; // Latin Capital Letter E with caron
  case 0x011c:
    return 'G'; // Latin Capital Letter G with circumflex
  case 0x011d:
    return 'g'; // Latin Small Letter G with circumflex
  case 0x011e:
    return 'G'; // Latin Capital Letter G with breve
  case 0x011f:
    return 'g'; // Latin Small Letter G with breve
  case 0x0120:
    return 'G'; // Latin Capital Letter G with dot above
  case 0x0121:
    return 'g'; // Latin Small Letter G with dot above
  case 0x0122:
    return 'G'; // Latin Capital Letter G with cedilla
  case 0x0123:
    return 'g'; // Latin Small Letter G with cedilla
  case 0x0124:
    return 'H'; // Latin Capital Letter H with circumflex
  case 0x0125:
    return 'h'; // Latin Small Letter H with circumflex
  case 0x0126:
    return 'H'; // Latin Capital Letter H with stroke
  case 0x0127:
    return 'h'; // Latin Small Letter H with stroke
  case 0x00ed:
    return 'i'; // Latin Small Letter I with acute
  case 0x00cd:
    return 'I'; // Latin Capital Letter I with acute
  case 0x00ee:
    return 'i'; // Latin Small Letter I with circumflex
  case 0x00ce:
    return 'I'; // Latin Capital Letter I with circumflex
  case 0x00ec:
    return 'i'; // Latin Small Letter I with grave
  case 0x00cc:
    return 'I'; // Latin Capital Letter I with grave
  case 0x00ef:
    return 'i'; // Latin Small Letter I with umlaut
  case 0x00cf:
    return 'I'; // Latin Capital Letter I with umlaut
  case 0x0129:
    return 'i'; // Latin Small Letter I with tilde
  case 0x0128:
    return 'I'; // Latin Capital Letter I with tilde
  case 0x012b:
    return 'i'; // Latin Small Letter I with macron
  case 0x012a:
    return 'I'; // Latin Capital Letter I with macron
  case 0x012d:
    return 'i'; // Latin Small Letter I with breve
  case 0x012c:
    return 'I'; // Latin Capital Letter I with breve
  case 0x012f:
    return 'i'; // Latin Small Letter I with ogonek
  case 0x012e:
    return 'I'; // Latin Capital Letter I with ogonek
  case 0x0131:
    return 'i'; // Latin Small Letter dotless I
  case 0x0130:
    return 'I'; // Latin Capital Letter I with dot above
  case 0x0134:
    return 'J'; // Latin Capital Letter J with circumflex
  case 0x0135:
    return 'j'; // Latin Small Letter J with circumflex
  case 0x0136:
    return 'K'; // Latin Capital Letter K with cedilla
  case 0x0137:
    return 'k'; // Latin Small Letter K with cedilla
  case 0x0138:
    return 'k'; // Latin Small Letter Kra
  case 0x0139:
    return 'L'; // Latin Capital Letter L with acute
  case 0x013a:
    return 'l'; // Latin Small Letter L with acute
  case 0x013b:
    return 'L'; // Latin Capital Letter L with cedilla
  case 0x013c:
    return 'l'; // Latin Small Letter L with cedilla
  case 0x013d:
    return 'L'; // Latin Capital Letter L with caron
  case 0x013e:
    return 'l'; // Latin Small Letter L with caron
  case 0x013f:
    return 'L'; // Latin Capital Letter L with middle dot
  case 0x0140:
    return 'l'; // Latin Small Letter L with middle dot
  case 0x0141:
    return 'L'; // Latin Capital Letter L with stroke
  case 0x0142:
    return 'l'; // Latin Small Letter L with stroke
  case 0x00f1:
    return 'n'; // Latin Small Letter N with tilde
  case 0x00d1:
    return 'N'; // Latin Capital Letter N with tilde
  case 0x0143:
    return 'N'; // Latin Capital Letter N with acute
  case 0x0144:
    return 'n'; // Latin Small Letter N with acute
  case 0x0145:
    return 'N'; // Latin Capital Letter N with cedilla
  case 0x0146:
    return 'n'; // Latin Small Letter N with cedilla
  case 0x0147:
    return 'N'; // Latin Capital Letter N with caron
  case 0x0148:
    return 'n'; // Latin Small Letter N with caron
  case 0x0149:
    return 'n'; // Latin Small Letter N preceded by apostrophe
  case 0x014a:
    return 'N'; // Latin Capital Letter Eng
  case 0x014b:
    return 'n'; // Latin Small Letter Eng
  case 0x00f3:
    return 'o'; // Latin Small Letter O with acute
  case 0x00d3:
    return 'O'; // Latin Capital Letter O with acute
  case 0x00f4:
    return 'o'; // Latin Small Letter O with circumflex
  case 0x00d4:
    return 'O'; // Latin Capital Letter O with circumflex
  case 0x00f2:
    return 'o'; // Latin Small Letter O with grave
  case 0x00d2:
    return 'O'; // Latin Capital Letter O with grave
  case 0x00f6:
    return 'o'; // Latin Small Letter O with umlaut
  case 0x00d6:
    return 'O'; // Latin Capital Letter O with umlaut
  case 0x00f5:
    return 'o'; // Latin Small Letter O with tilde
  case 0x00d5:
    return 'O'; // Latin Capital Letter O with tilde
  case 0x00f8:
    return 'o'; // Latin Small Letter O with stroke
  case 0x00d8:
    return 'O'; // Latin Capital Letter O with stroke
  case 0x0151:
    return 'o'; // Latin Small Letter O with double acute
  case 0x0150:
    return 'O'; // Latin Capital Letter O with double acute
  case 0x0153:
    return 'o'; // Latin Small Ligature OE
  case 0x0152:
    return 'O'; // Latin Capital Ligature OE
  case 0x014d:
    return 'o'; // Latin Small Letter O with macron
  case 0x014c:
    return 'O'; // Latin Capital Letter O with macron
  case 0x014f:
    return 'o'; // Latin Small Letter O with breve
  case 0x014e:
    return 'O'; // Latin Capital Letter O with breve
  case 0x0154:
    return 'R'; // Latin Capital Letter R with acute
  case 0x0155:
    return 'r'; // Latin Small Letter R with acute
  case 0x0156:
    return 'R'; // Latin Capital Letter R with cedilla
  case 0x0157:
    return 'r'; // Latin Small Letter R with cedilla
  case 0x0158:
    return 'R'; // Latin Capital Letter R with caron
  case 0x0159:
    return 'r'; // Latin Small Letter R with caron
  case 0x015a:
    return 'S'; // Latin Capital Letter S with acute
  case 0x015b:
    return 's'; // Latin Small Letter S with acute
  case 0x00df:
    return 's'; // Latin Small Letter sharp S
  case 0x015c:
    return 'S'; // Latin Capital Letter S with circumflex
  case 0x015d:
    return 's'; // Latin Small Letter S with circumflex
  case 0x015e:
    return 'S'; // Latin Capital Letter S with cedilla
  case 0x015f:
    return 's'; // Latin Small Letter S with cedilla
  case 0x0160:
  ...
  case 0x0162:
    return 'T'; // Latin Capital Letter T with cedilla
  case 0x0163:
    return 't'; // Latin Small Letter T with cedilla
  case 0x0164:
    return 'T'; // Latin Capital Letter T with caron
  case 0x0165:
    return 't'; // Latin Small Letter T with caron
  case 0x0166:
    return 'T'; // Latin Capital Letter T with stroke
  case 0x0167:
    return 't'; // Latin Small Letter T with stroke
  case 0x00fa:
  ...
  case 0x00fc:
    return 'u'; // Latin Small Letter U with umlaut
  case 0x00dc:
    return 'U'; // Latin Capital Letter U with umlaut
  case 0x0171:
    return 'u'; // Latin Small Letter U with double acute
  case 0x0170:
    return 'U'; // Latin Capital Letter U with double acute
  case 0x0169:
    return 'u'; // Latin Small Letter U with tilde
  case 0x0168:
    return 'U'; // Latin Capital Letter U with tilde
  case 0x016b:
    return 'u'; // Latin Small Letter U with macron
  case 0x016a:
    return 'U'; // Latin Capital Letter U with macron
  case 0x016d:
    return 'u'; // Latin Small Letter U with breve
  case 0x016c:
    return 'U'; // Latin Capital Letter U with breve
  case 0x0173:
    return 'u'; // Latin Small Letter U with ogonek
  case 0x0172:
    return 'U'; // Latin Capital Letter U with ogonek
  case 0x016f:
    return 'u'; // Latin Small Letter U with ring above
  case 0x016e:
    return 'U'; // Latin Capital Letter U with ring above
  case 0x0174:
    return 'W'; // Latin Capital Letter W with circumflex
  case 0x0175:
    return 'w'; // Latin Small Letter W with circumflex
  case 0x00ff:
    return 'y'; // Latin Small Letter Y with umlaut
  case 0x0178:
    return 'Y'; // Latin Capital Letter Y with umlaut
  case 0x0177:
    return 'y'; // Latin Small Letter Y with circumflex
  case 0x0176:
    return 'Y'; // Latin Capital Letter Y with circumflex
  case 0x0179:
    return 'Z'; // Latin Capital Letter Z with acute
  case 0x017a:
    return 'z'; // Latin Small Letter Z with acute
  case 0x017b:
    return 'Z'; // Latin Capital Letter Z with dot above
  case 0x017c:
    return 'z'; // Latin Small Letter Z with dot above
  case 0x017d:
...
pbyte PAllocate(long cb, CONST char *szType) {
  ...
  Assert(szType != NULL);
  if (pb == NULL) {
    sprintf(szT, "%s: Not enough memory for %s (%ld bytes).", szAppName, szType,
            cb);
    PrintWarning(szT);
  } else {
    is.cAlloc++;
...
flag FEnsureMacro(int cszNew) {
  ...
  if (cszNew <= 48)
    cszNew = 48 + 1;
  rgszT = (char **)RgReallocate(is.rgszMacro, is.cszMacro, sizeof(char *),
                                cszNew, "macro list");
  if (rgszT == NULL)
    return fFalse;
