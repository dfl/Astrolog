...
KV BmpGetXY(Bitmap *b, int x, int y) { return _GetXY(b, x, y); }
void SetXY(int x, int y, KI ki) {
  if (!gi.fBmp)
    BmSet(gi.bm, x, y, ki);
  else
    BmpSetXY(&gi.bmp, x, y, ki);
}
...
void BmpCopyBlock(CONST Bitmap *bs, int x1, int y1, int x2, int y2, Bitmap *bd,
                  int x3, int y3, int x4, int y4) {
  int xs = x2 - x1 + 1, ys = y2 - y1 + 1, xd = x4 - x3 + 1, yd = y4 - y3 + 1, x,
      y, xT, yT, nR, nG, nB;
  byte *pbDst;
...
void BmpCopyBlock2(CONST Bitmap *bs, real x1, real y1, real x2, real y2,
  ...
  if (gs.fMollewide) {
    ...
    if (us.fEclipse && NCheckEclipseSolar(oEar, oMoo, oSun, NULL) > etNone)
      fDoEclipse = fTrue;
  }
...
void BmpCopyWin(CONST Bitmap *b, HDC hdc, int x, int y) {
  ...
  SetDIBitsToDevice(hdc, x, y, b->x, b->y, 0, 0, 0, b->y, b->rgb,
                    (BITMAPINFO *)&bi, DIB_RGB_COLORS);
}
#endif
#ifdef FLTK
// Copy a 24 bit bitmap structure to an FLTK drawing area.
void BmpCopyFltk(CONST Bitmap *b, int x, int y) {
  byte *p, *pRes, *pb;
  int m, n;

  // FLTK's fl_draw_image() expects RGB, while Astrolog's Bitmap is BGR.
  // We need to create a temporary buffer for the correctly ordered pixels.
  pRes = (byte *)PAllocate(b->x * b->y * 3, "FLTK blit buffer");
  if (pRes == NULL)
    return;
  for (n = 0; n < b->y; n++) {
    pb = _PbXY(b, 0, n);
  ...
  fl_draw_image(pRes, x, y, b->x, b->y, 3, 0);
  DeallocateP(pRes);
}
#endif // Draw the background bitmap onto the specified 24 bit bitmap.
       // Implements the
// -XI switch features.
...
flag FBmpDrawBack(Bitmap *bDest) {
  ...
  if (gi.fFile || bDest != NULL) {
    if ((gs.ft == ftBmp && gi.fBmp) || bDest != NULL)
      BmpCopyBlock(&gi.bmpBack2, x3, y3, x3 + x4 - 1, y3 + y4 - 1,
                   bDest != NULL ? bDest : &gi.bmp, x1, y1, x1 + x2 - 1,
                   y1 + y2 - 1);
    return fTrue;
  }
  ...
  if (wi.hdcBack == NULL) {
    ...
    SetWindowOrg(wi.hdcBack, 0, 0);
    SetViewportOrg(wi.hdcBack, 0, 0);
  }
...
flag FBmpDrawMap() {
  ...
  real deg = Mod(rDegMax - gs.rRot), lonS, latS, rxc, ryc, rzc, lon, lat, lat0,
       rT, rLen, sint, cost, sina, cosa;
  KV kv;
  flag fDoEclipse = fFalse;
  ...
  if (gi.bmpWorld.rgb == NULL && !FLoadBmp(BITMAP_EARTH, &gi.bmpWorld, fFalse))
    return fFalse;
#if defined(WINANY) || defined(FLTK)
  if (!gi.fFile) {
#ifdef WINANY
    ...
    bmp = &wi.bmpWin;
#else
    if (!FAllocateBmp(&fi.bmpWin, gs.xWin, gs.yWin))
      return fFalse;
    bmp = &fi.bmpWin;
#endif
  }
  ...
  xc = (gs.xWin >> 1) - !FOdd(gs.xWin);
  yc = (gs.yWin >> 1) - !FOdd(gs.yWin);
  zc = Max(xc, yc);
  rxc = (real)xc;
  ryc = (real)yc;
  rzc = (real)zc;
  ...
  if (gi.nMode == gAstroGraph || (gi.nMode == gWorldMap && !gs.fMollewide)) {
    if (x1 == 0 || x2 == 0)
      ...
      BmpCopyBlock(&gi.bmpWorld, 0, 0, x1 - 1, gi.bmpWorld.y - 1, bmp,
                   gs.xWin - x2 - 1, 0, gs.xWin - 1, gs.yWin - 1);
    ...
    for (y2 = 0; y2 < gs.yWin; y2++) {
      ...
      if (x1 == 0 || x2 == 0)
        BmpCopyBlock(&gi.bmpWorld, 0, y1, gi.bmpWorld.x - 1, y1, bmp, n, y2,
                     gs.xWin - 1 - n, y2);
      else {
        n2 = (int)(rT * (real)(x2 - xc) / rDegHalf);
        n2 += xc;
        BmpCopyBlock(&gi.bmpWorld, 0, y1, x1 - 1, y1, bmp, gs.xWin - n2 - 1, y2,
                     gs.xWin - 1 - n, y2);
        BmpCopyBlock(&gi.bmpWorld, x1, y1, gi.bmpWorld.x - 1, y1, bmp, n, y2,
                     gs.xWin - n2, y2);
      }
    }
    ...
    for (y1 = 0; y1 < gs.yWin; y1++) {
      ...
      for (x1 = 0; x1 < gs.xWin; x1++) {
        ...
        if (gs.rTilt != 0.0) {
          lat = rDegQuad - lat;
          CoorXformFast(&lon, &lat, RSinD(lon), RCosD(lon), sina, cosa, sint,
                        cost);
          lat = rDegQuad - lat;
        }
        lon = Mod(lon - gs.rRot);
...
flag FBmpDrawMap2(int x1, int y1, int x2, int y2, real rx1, real ry1, real rx2,
                  real ry2) {
  Bitmap *bmp = &gi.bmp;
  int x12;
...
#endif
  rx = (real)gi.bmpWorld.x / rDegMax;
  ry = (real)gi.bmpWorld.y / rDegHalf;
  BmpSetAll(bmp, rgbbmp[gi.kiOff]);
  rx1 = Mod(rx1);
  rx2 = Mod(rx2);
  x3 = rx1 * rx;
...
void WriteXBitmap(FILE *file, CONST char *szName, char mode) {
  ...
  for (pchEnd = szT; *pchEnd != chNull; pchEnd++)
    ;
  for (pchStart = pchEnd;
       pchStart > szT && *(pchStart - 1) != '/' && *(pchStart - 1) != '\\';
       pchStart--)
    ;
  ...
  *pchEnd = chNull;
  ...
  fprintf(file, "static %s %s_bits[] = {", mode != 'V' ? "char" : "short",
          pchStart);
  for (y = 0; y < gs.yWin; y++) {
    x = 0;
    do {
...
void PsFont(int nFont) {
  ...
  if (nFont == fiArial) {
    szFont = "Times-Roman";
    z = 4 * PSMUL * gi.nScaleText;
  } else if (nFont == fiCourier || nFont == fiAstrolog) {
    szFont = "Courier";
    z = 5 * PSMUL * gi.nScaleText;
  }
...
void MetaInit() {
  ...
  MetaWord(16 * 5 + 1 +
           (gs.nFontAll > 0) * (cFont - 1)); // Number of objects in metafile
  ...
  for (j = 1; j <= 4; j++)
    for (i = 0; i < 16; i++) {
      ...
      MetaCreatePen(j <= 2 ? 0 : j - 2 /* PS_SOLID; PS_DASH; PS_DOT */, k,
                    rgbbmp[i]);
    }
  for (i = 0; i < 16; i++) {
  ...
  if (gs.nFontAll > 0)
    for (i = 1; i < cFont; i++) {
      ...
      MetaCreateFont(
          j, 0, i < fiCourier ? -METAMUL * gi.nScale : yFontT,
          i == fiWingding ? 2 /* Symbol Charset */ : 0 /* Ansi Charset */);
      j = (i == fiWingding || i >= fiCourier ? 1 /* Draft */ : 0 /* Default */);
...
void WireDrawGlobe(flag fSky, real deg) {
  int X[objMax], Y[objMax], Z[objMax], M[objMax], N[objMax], O[objMax], rz,
      unit = 12 * gi.nScale, x, y, z, xold, yold, zold, m, n, o, u, v, w, i, j,
      k, l;
...
void WireChartOrbit() {
  ...
  if (!gs.fHouseExtra) {
    ...
    for (i = 0; i < cSign; i++) {
      ...
      if (gs.fColorSign)
        DrawColor(kSignB(j));
      xd = NCosD(k, nDegHalf - i * 30);
      yd = NSinD(k, nDegHalf - i * 30);
      WireLine(0, 0, 0, xd, yd, 0);
      xd = NCosD(k, nDegHalf - i * 30 - 15);
      yd = NSinD(k, nDegHalf - i * 30 - 15);
      DrawSign(j, xd, yd);
    }
  }
  ...
  for (i = 0; i <= is.nObj; i++)
    if (FProper(i)) {
      ...
      for (j = 0; j < gi.cspace - 1; j++) {
        ...
        k = gi.cspace - j;
        WireLine(-(int)(xp * sx), (int)(yp * sx),
                 (int)(zp * sx) - k * gs.zspace, -(int)(xp2 * sx),
                 (int)(yp2 * sx), (int)(zp2 * sx) - (k - 1) * gs.zspace);
      }
...
void WireChartSphere() {
  char sz[cchSzDef];
  int rgx[objMax], rgy[objMax], rgz[objMax], zGlyph, cChart, iChart, zr,
      xo = 0, yo = 0, zo = 0, xp, yp, zp, i, j, k, k2, nSav;
  ...
  for (j = -1; j <= cSign; j++) {
    if (!(!gs.fHouseExtra && fHouse3D) && !(j <= 0 && !fNoHorizon))
      continue;
    if (fHouse3D && ((j == sCap && chouse[j] == is.MC) ||
      ...
      if (i > -90) {
        ...
        if (j <= 0) {
          ...
          for (k2 = -k; k2 <= k; k2 += (k << 1)) {
            WireSphereMeridian((real)(j == 0 ? i + 180 : 360 - i),
                               (real)k2 / 2.0, zr, &xo, &yo, &zo);
            WireLine(xo, yo, zo, xp, yp, zp);
          }
        }
      }
      ...
      for (i = j - 1; i >= 0; i--)
        if (grid->n[i][j] && FProper(i) && FProper(j)) {
          ...
          if (gs.fLabelAsp) {
            ...
            DrawAspect(grid->n[i][j], (rgx[i] + rgx[j]) >> 1,
                       (rgy[i] + rgy[j]) >> 1);
          }
        }
