...
flag FOutputDaedalusStar()
{
  ...
  file = fopen(is.szFileOut, "w");  // Create and open the file for output.
  if (file == NULL) {
    sprintf(sz, "Daedalus file '%s' can not be created.", is.szFileOut);
    PrintError(sz);
    return fFalse;
  }
  fprintf(file, "DS\n// Created by %s %s\nIf Lt Version 3400 "
    "{Message 'This script requires Daedalus 3.4 or later to run.' Halt}\n",
    szAppName, szVersionCore);
  ...
  if (gs.fAllStar) {
    ...
    while (SwissComputeStar(is.T, &es)) {
      k = 255 - (int)((es.mag - rStarLite) / rStarSpan * 224.0);
      k = Min(k, 255); k = Max(k, 32);
      fprintf(file, "SetStar 1 %d %d %d GrayN %d\n", j,
        (int)(Mod(270.0 - es.lon) * 32768.0 / rDegMax),
        (int)((es.lat + rDegQuad) * 32768.0 / rDegHalf), k);
      j++;
    }
  }

  // Output extra asteroids.
  if (gs.nAstLo > 0) {
  ...
  if (gs.fAllExo) {
    ...
    while (EnumExoplanets(&es)) {
      ...
      fprintf(file, "SetStar 1 %d %d %d %s\n", j,
        (int)(Mod(270.0 - es.lon) * 32768.0 / rDegMax),
        (int)((es.lat + rDegQuad) * 32768.0 / rDegHalf),
        es.ki == kGreen ? "Green" : "Maize");
      j++;
    }
  }

  fprintf(file, "nStarCount %d\nnStarSize 384\nfStars True\n", j);
  fclose(file);
  return fTrue;
}
#endif
...
// switch file, which may be read in to restore those settings. Usually this
// is used to create a new astrolog.as default settings file. This is called
// from the -od switch and the File / Save Program Settings menu command.
...
flag FOutputSettings()
{
  ...
  sprintf(sz, "-zf %s                 ", SzTemperature(us.tmpDef)); PrintFSz();
  ...
  for (i = 0; *rgZodiacOffset[i].sz; i++) {
    if (us.rZodiacOffset == rgZodiacOffset[i].r) {
      sprintf(sz, "%.4s", rgZodiacOffset[i].sz);
      break;
    }
  }
  PrintFSz();
  PrintF(
    " ; Sidereal zodiac offset    [Change \"Faga\" to desired ayanamsa ]\n");
  ...
  sprintf(sz, "-YR2 %s%s ; "
    "Restrict latitude zero node crossings, distance equivalence\n\n",
  ...
  for (i = starLo; i <= starHi; i++) {
    ...
    fAny = fTrue;
    if (f1) {
      sprintf(sz, "-YU %d %s%s", i, szStarCustom[j], f2 ? " " : "\n");
      PrintFSz();
    }
    if (f2) {
      sprintf(sz, "-YD %d \"%s\"\n", i, szObjDisp[i]); PrintFSz();
  ...
  sprintf(sz, "%cXJ              ", ChDashF(gs.fIndianWheel)); PrintFSz();
  PrintF("; Indian type wheels [\"_XJ\" is Western, \"=XJ\" is Indian  ]\n");
...
real RParseSz(CONST char *szEntry, int pm)
{
  ...
  if (pm == pmTim) {
    ...
    // Check for "F" or "K" suffix for Fahrenheit/Kelvin instead of Celcius.
    i = Max(cch-1, 0);
    if (sz[i] == 'F')
      r = (r - 32.0) * 5.0/9.0;
    else if (sz[i] == 'K')
...
flag FInputData(CONST char *szFile)
{
  ...
  if (szFile[0] == '_' && szFile[1] == '_') {
    ch = szFile[2];
    if (FBetween(ch, '1', '0' + cRing) && szFile[3] == chNull) {
      is.fHaveInfo = fTrue;
      ciCore = *rgpci[ch - '0'];
      return fTrue;
    }
