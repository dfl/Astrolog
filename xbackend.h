...
typedef struct _GraphicsBackend {
  ...
  void (*PutLineF)(double x1, double y1, double x2,
                   double y2); // Draw line (float coords)
  ...
  int (*PutText)(const char *sz, int x, int y, int nFont,
                 int nScale); // Draw text
...
extern void FltkAdjustMapGlyphScale(int delta);
...
extern cairo_t *
CairoContext(void); // Get current Cairo context for advanced operations
#endif
...
#define GBSetColor(ki)                                                         \
  do {                                                                         \
    if (gpBackend && gpBackend->PutColor)                                      \
...
#define GBSetLineWidth(width)                                                  \
  do {                                                                         \
    if (gpBackend && gpBackend->PutLineWidth)                                  \
      gpBackend->PutLineWidth(width);                                          \
  } while (0)

#define GBDrawPixel(x, y)                                                      \
  do {                                                                         \
    if (gpBackend && gpBackend->PutPixel)                                      \
      gpBackend->PutPixel(x, y);                                               \
  } while (0)

#define GBDrawPixelThick(x, y)                                                 \
  do {                                                                         \
    if (gpBackend && gpBackend->PutPixelThick)                                 \
      gpBackend->PutPixelThick(x, y);                                          \
  } while (0)

#define GBDrawLine(x1, y1, x2, y2)                                             \
  do {                                                                         \
    if (gpBackend && gpBackend->PutLine)                                       \
      gpBackend->PutLine(x1, y1, x2, y2);                                      \
  } while (0)

#define GBDrawLineF(x1, y1, x2, y2)                                            \
  do {                                                                         \
    if (gpBackend && gpBackend->PutLineF)                                      \
      gpBackend->PutLineF(x1, y1, x2, y2);                                     \
    else if (gpBackend && gpBackend->PutLine)                                  \
      gpBackend->PutLine((int)(x1), (int)(y1), (int)(x2), (int)(y2));          \
  } while (0)

#define GBDrawLineThick(x1, y1, x2, y2)                                        \
  do {                                                                         \
    if (gpBackend && gpBackend->PutLineThick)                                  \
      gpBackend->PutLineThick(x1, y1, x2, y2);                                 \
  } while (0)

#define GBDrawRect(x1, y1, w, h)                                               \
  do {                                                                         \
    if (gpBackend && gpBackend->PutRect)                                       \
      gpBackend->PutRect(x1, y1, w, h);                                        \
  } while (0)

#define GBDrawArc(x, y, w, h, d1, d2)                                          \
  do {                                                                         \
    if (gpBackend && gpBackend->PutArc)                                        \
      gpBackend->PutArc(x, y, w, h, (double)(d1), (double)(d2));               \
  } while (0)
...
#define GBClearScreen(ki)                                                      \
  do {                                                                         \
    if (gpBackend && gpBackend->ClearScreen)                                   \
      gpBackend->ClearScreen(ki);                                              \
  } while (0)

#define GBFlush()                                                              \
  do {                                                                         \
    if (gpBackend && gpBackend->Flush)                                         \
      gpBackend->Flush();                                                      \
  } while (0)
...
#define GBDrawGlyph(ch, x, y, nFont, nScale)                                   \
  (gpBackend && gpBackend->PutGlyph                                            \
       ? gpBackend->PutGlyph(ch, x, y, nFont, nScale)                          \
       : 0)

#define GBDrawText(sz, x, y, nFont, nScale)                                    \
  (gpBackend && gpBackend->PutText                                             \
       ? gpBackend->PutText(sz, x, y, nFont, nScale)                           \
       : 0)
