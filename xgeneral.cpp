...
void DrawDash(int x1, int y1, int x2, int y2, int skip, flag fAspect) {
  ...
  if (!gi.fFile) {
    ...
    if (gs.nDashStyle >= 1 && (skip > 0 || fAspect)) {
      ...
      // "Exaggerated (less alpha for weak aspects)"
      // Apply power curve to the density
      double alphaNorm = pow(base_density, 3.0);

      int alpha = (int)(40 + alphaNorm * 215); // Map to 40-255
      ...
      double width =
          fAspect ? (0.5 + alphaNorm * 1.5) : 1.0; // 0.5 to 2.0 for aspects
