...
/*
******************************************************************************
** OKLCH Color Space Conversion (for perceptually uniform lightness adjustment)
******************************************************************************
*/
...
// RGB (0-255) to Oklab L,a,b
static void RgbToOklab(int r, int g, int b, double *L, double *a,
                       double *ok_b) {
  // sRGB to linear
...
// Oklab L,a,b to RGB (0-255)
static void OklabToRgb(double L, double a, double ok_b, int *r, int *g,
                       int *b) {
  // Oklab to LMS
  double l_ = L + 0.3963377774 * a + 0.2158037573 * ok_b;
  double m_ = L - 0.1055613458 * a - 0.0638541728 * ok_b;
  double s_ = L - 0.0894841775 * a - 1.2914855480 * ok_b;
  ...
  // Linear to sRGB, clamp, and convert to 0-255
  double sr = LinearToSrgb(lr);
  double sg = LinearToSrgb(lg);
  double sb = LinearToSrgb(lb);

  // Clamp to valid range
  *r = (int)(Max(0.0, Min(1.0, sr)) * 255.0 + 0.5);
  *g = (int)(Max(0.0, Min(1.0, sg)) * 255.0 + 0.5);
  *b = (int)(Max(0.0, Min(1.0, sb)) * 255.0 + 0.5);
}

// Adjust color lightness using OKLCH (perceptually uniform)
// factor: 0.0 = black, 1.0 = original color
static KV AdjustLightnessOklch(KV kv, double factor) {
  double L, a, ok_b;
  RgbToOklab(RgbR(kv), RgbG(kv), RgbB(kv), &L, &a, &ok_b);

  // Scale lightness (in Oklab, L is perceptually uniform)
  L *= factor;

  int r, g, b;
  OklabToRgb(L, a, ok_b, &r, &g, &b);
  ...
  if (gi.fFile) {
    ...
    if (gs.ft == ftPS) {
      if (gi.kiCur != col) {
        ...
        fprintf(gi.file, "%.2f %.2f %.2f c\n", (real)RgbR(rgbbmp[col]) / 255.0,
                (real)RgbG(rgbbmp[col]) / 255.0,
...
void DrawSpot(int x, int y) {
  ...
  if (gs.ft == ftWmf) {
    ...
    MetaEllipse(x - gi.nPenWid * 2, y - gi.nPenWid * 2,
                x + gi.nPenWid * (3 + gs.fThick),
                y + gi.nPenWid * (3 + gs.fThick));
    return;
  }
#endif
#ifdef WIRE
...
void DrawBlock(int x1, int y1, int x2, int y2) {
  ...
  if (gi.fFile) {
    if (gs.ft == ftBmp) {
      ...
      DrawColor(gi.kiCur);
      fprintf(gi.file, "%d %d %d %d rf\n", Max(x1 - gi.nPenWid / 4, 0),
              Max(y1 - gi.nPenWid / 4, 0), x2 - x1 + gi.nPenWid / 4,
              y2 - y1 + gi.nPenWid / 4);
    }
#endif
#ifdef META
      ...
      MetaSelect();
      MetaRectangle(x1 - gi.nPenWid, y1 - gi.nPenWid, x2 + gi.nPenWid,
                    y2 + gi.nPenWid);
    }
#endif
#ifdef WIRE
...
#ifdef EXPRESS
void AdjustGlyph(int *ch, int *x, int *y, int *fi, int *nScale, int i,
                 char *szExpFont) {
...
flag DrawGlyph(int ch, int x, int y, int fi, int nScale) {
  ...
  // Fall back to Windows GDI font rendering
  {
    ...
    hfont =
        CreateFont(12 * gi.nScale * nScale / 100, 0, 0, 0,
                   !gs.fThick ? 400 : 800, fFalse, fFalse, fFalse,
                   FBetween(fi, fiCourier, fiArial)
                       ? DEFAULT_CHARSET
                       : (fi >= fiAstro && fi != fiNakshatr ? ANSI_CHARSET
                                                            : SYMBOL_CHARSET),
    ...
    if (!gi.fEps) {
      ...
      if (gs.nOrient < 0) {
        ...
        rT = gs.xInch / (real)gs.yWin < gs.yInch / (real)gs.xWin
                 ? gs.xInch * 72.0 / (real)gs.yWin
                 : gs.yInch * 72.0 / (real)gs.xWin;
        ...
        rT = gs.xInch / (real)gs.xWin < gs.yInch / (real)gs.yWin
                 ? gs.xInch * 72.0 / (real)gs.xWin
                 : gs.yInch * 72.0 / (real)gs.yWin;
        fprintf(gi.file, "%d %d translate\n",
...
void DrawLineF(real x1, real y1, real x2, real y2) {
  ...
  DrawLine((int)(x1 + rRound), (int)(y1 + rRound), (int)(x2 + rRound),
           (int)(y2 + rRound));
}
...
// fAspect indicates if this is an aspect line (for ellipse drawing).

void DrawDash(int x1, int y1, int x2, int y2, int skip, flag fAspect) {
  int x = x1, y = y1, i = 0, dx = x2 - x1, dy = y2 - y1, xInc, yInc, xInc2,
      yInc2, d, dInc, z, zMax;
  ...
  if (!gi.fFile) {
    ...
    // Only enter for:
    //   - Aspect lines (fAspect=true), regardless of skip value
    //   - Dashed non-aspect lines (skip > 0)
    // Do NOT enter for solid non-aspect lines (skip=0, fAspect=false) like
    // vector text!
    if (gs.nDashStyle >= 1 && (skip > 0 || fAspect)) {
      // Convert skip to strength (0=weakest, 1=strongest)
      // "Map more closely to how the stippled lines look"
      // Stippled lines use a skip factor of nDash = Orb_deg / 2.
      // Density is 1 / (1 + nDash).
      // 'skip' here is passed in 0.1 degree steps (from xcharts0.cpp).
      // So stippled_skip = skip / 20.0.
      double stippled_skip = (double)skip / 20.0;
      double base_density = 1.0 / (1.0 + stippled_skip);

      // "Exaggerated (less visible for weak aspects)"
      // Apply power curve to the density
      double strengthNorm = pow(base_density, 3.0);

      // Use alpha transparency for aspect line fading
      // (OKLCH conversion functions kept in this file for potential future use)
      int alpha = (int)(40 + strengthNorm * 215); // Map to 40-255
      GBSetColorAlpha(gi.kiCur, alpha);

      // Variable width ONLY for aspect lines, not for other dashed lines
      // This prevents house cusps, vector text, etc. from getting thick
      double width =
          fAspect ? (0.5 + strengthNorm * 1.5) : 1.0; // 0.5 to 2.0 for aspects
      if (fAspect && gs.fThick)
        width *= 1.5; // Scale up further if Thick Lines enabled

      // Draw as rotated ellipse instead of line for tapered ends
      ...
      double cx = (x1 + x2) / 2.0;
      double cy = (y1 + y2) / 2.0;
      double angle = atan2(dy, dx); // Rotation angle in radians

      // Ellipse radii: rx = half length, ry = width (creates tapered effect)
      double rx = length / 2.0;
      double ry = width;

      // Only draw ellipses for actual aspect lines (fAspect=fTrue)
      // Connector lines, house cusps, and angles use fAspect=fFalse
      // Also require minimum length to avoid degenerate Cairo transformation
      // (cairo_scale with 0 or tiny values corrupts the context)
      flag fUseEllipse = fAspect && (rx > 1.0) && (ry > 0.1);
#ifdef CAIRO
      // Use Cairo for rotated ellipse with transform
      if (fUseEllipse) {
        ...
        if (cr) {
          ...
          cairo_fill(cr); // Fill BEFORE restore so transform is applied
          cairo_restore(cr);
          GBSetLineWidth(1.0);            // Restore default line width
          GBSetColorAlpha(gi.kiCur, 255); // Explicitly reset to full opacity
          return;
        }
      }
#endif

      // Fallback to line for non-Cairo backends
...
// Wrapper for drawing aspect lines with ellipse support
void DrawAspectDash(int x1, int y1, int x2, int y2, int skip) {
  DrawDash(x1, y1, x2, y2, skip, fTrue);
}
...
void DrawWrap(int x1, int y1, int x2, int y2, int xmin, int xmax) {
  ...
  ymid =
      y1 + (int)((real)(y2 - y1) *
                     (!j && x1 < xmid ? (real)(x1 - xmin) : (real)(xmax - x1)) /
                     (real)i +
                 rRound);
...
#define RotateR2(x1, y1, x2, y2, rS, rC)                                       \
  x2 = (x1) * (rC) - (y1) * (rS);                                              \
  y2 = (y1) * (rC) + (x1) * (rS)
#define RotateR2Init(rS, rC, d)                                                \
  rS = RSinD(d);                                                               \
  rC = RCosD(d)
...
void DrawSz(CONST char *sz, int x, int y, int dt) {
  int nFont = gs.nFontTxt, nScaleSav = gi.nScale, kiSav = gi.kiCur, nScale2,
      cch, i, dch;
  ...
  if (dt & dtErase) {
    DrawColor(gi.kiOff);
    DrawBlock(x, y, x + xFont * nScale2 * cch / 2 - 1,
              y + (yFont - 2) * nScale2 / 2);
  }
  DrawColor(kiSav);
#ifdef PS
...
#ifdef WINANY
  if (!gi.fFile && nFont > 0) {
    hfont = CreateFont(6 * nScale2, 0, 0, 0, !gs.fThick ? 400 : 800, fFalse,
                       fFalse, fFalse, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                       CLIP_DEFAULT_PRECIS, PROOF_QUALITY,
                       FIXED_PITCH | FF_MODERN, rgszFontName[nFont]);
    if (hfont == NULL)
      return;
    hfontPrev = (HFONT)SelectObject(wi.hdc, hfont);
    kvSav = GetTextColor(wi.hdc);
  ...
  while (*sz) {
    ...
    if (!gi.fFile && nFont > 0) {
      if (dch > 1)
        TextOutW(wi.hdc, x - nScale2 / 2, y - nScale2 * 3 / 2, (LPCWSTR)&wch,
                 1);
      else
...
void DrawSign(int i, int x, int y) {
  int nFont = gs.nFontSig, ch = -1, nScale = 100;
  flag fDoThin;
  ...
  if (nFont == fiWingding)
    ...
    ch = szSignFontEnigma[i == sCap && gs.nGlyphCap > 1 ? cSign : i - 1];
    y -= gi.nScale;
  } else if (nFont == fiHamburg) {
    ch = szSignFontHamburg[i == sCap && gs.nGlyphCap > 1 ? cSign : i - 1];
    y += gi.nScale;
...
CONST uchar szObjectFontAstronomicon[dwarfHi + 2] =
    ...
    ">QRSTUVWXYZqlmnogiz?kjc23e56f89d;<"
    "b\241\242\243\244\245\246\247\250prstuvwx"
    "y";
CONST wchar wzObjectFontUnicode[oCore + 1] = {
    0 /*0x2295*/, 0x2609,     0x263d,     0x263f,     0x2640, 0x2642,
    0x2643,       0x2644,     0x2645,     0x2646,     0x2647, 0 /*26b7*/,
    0 /*26b3*/,   0 /*26b4*/, 0 /*26b5*/, 0 /*26b6*/, 0x260a, 0x260b,
    0 /*26b8*/,   0 /*2297*/, 0,          0};
...
void DrawObject(int obj, int x, int y) {
  ...
  if (nFont == fiAstro && obj <= cuspHi && szObjectFontAstro[obj] > ' ')
    ...
    if (obj == oUra && gs.nGlyphUra == 2)
      ...
      ch = '_';
    else if (obj == oPlu && gs.nGlyphPlu == 3)
      ch = '`';
...
// Draw a Nakshatra glyph or number at particular coordinates on the screen.

void DrawNakshatra(int i, int x, int y) {
  char sz[3];
...
#ifdef EXPRESS
  // Adjust color if AstroExpression says to do so.
  if (!us.fExpOff && FSzSet(us.szExpCity)) {
    ExpSetN(iLetterW, iae);
    ExpSetR(iLetterX, is.rgae[iae].lon);
    ExpSetR(iLetterY, is.rgae[iae].lat);
    ExpSetN(iLetterZ, ki);
