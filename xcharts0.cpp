...
void DrawInfo(CI *pci, CONST char *szHeader, flag fAll) {
  ...
  if (FNoTimeOrSpace(*pci))
    ...
    if (!f1K)
      ...
      pch[14] = chNull;
      sprintf(sz, "Longitude: %s", pch);
      DrawPrint(sz, gi.kiLite, fFalse);
      sprintf(sz, "Latitude:   %s", pch + 15);
      DrawPrint(sz, gi.kiLite, fFalse);
    }
    if (fAll) {
      sprintf(sz, "%s%s houses",
              us.fHouse3D == (gi.nMode != gSphere)
                  ? "3D "
                  : (us.fHouse3D && gi.nMode == gSphere ? "2D " : ""),
              szSystem[is.nHouseSystem]);
      DrawPrint(sz, gi.kiLite, fFalse);
      sprintf(sz, "%s, %s",
              !us.fSidereal ? "Tropical"
      ...
      if (us.rObjAddition != 0.0) {
        sprintf(sz, "%s: %s (%s)",
                us.rObjAddition != us.rCuspAddition ? "Objs" : "Time",
                SzHMS((int)(us.rObjAddition * 3600.0 +
                            rSmall * RSgn2(us.rObjAddition)) *
                      60),
                SzTim(Tim + us.rObjAddition));
        DrawPrint(sz, gi.kiLite, fFalse);
      }
      if (us.rCuspAddition != 0.0 && us.rCuspAddition != us.rObjAddition) {
        sprintf(sz, "Cusp: %s (%s)",
                SzHMS((int)(us.rCuspAddition * 3600.0 +
                            rSmall * RSgn2(us.rCuspAddition)) *
                      60),
                SzTim(Tim + us.rCuspAddition));
        DrawPrint(sz, gi.kiLite, fFalse);
      }
...
void DrawSidebar() {
  ...
  if (!gs.fIndianWheel) {
    if (gs.nDecaType == 1) {
      ...
      for (y = 0; y <= 1; y++)
        for (i = 0; i <= 1; i++)
          for (a = 1; a < j; a++)
            DrawLine(i * (gs.xWin - 1),
                     y ? (gs.yWin - 1 - a * k / j) : a * k / j,
                     i ? gs.xWin - 1 - l + a * l / j : l - a * l / j,
                     y * (gs.yWin - 1));
      ...
      if (gs.nDecaType <= 4) {
        ...
        if (gi.nScale > 0) {
          ...
          for (i = 0; i < 5; i++) {
            ...
            DrawColor(kRayB[rays % 10]);
            DrawSz(sz, 7, 7 + (4 - i) * gi.nScale * yFont,
  ...
  if (!gs.fText || !gs.fDoSidebar) // Don't draw sidebar if -Xv0 flag unset.
    return;
  a = us.fAnsiChar;
  us.fAnsiChar = (gs.nFontTxt == 0 || (gs.ft != ftPS && gs.ft != ftWmf)) << 1;
  DrawColor(gi.kiLite);
  i = gs.xWin - 1;
  gs.xWin += xSideT;
  if (gs.fBorder)
  ...
  if (us.nHouseSystem != hsNull) {
    ...
    for (i = 1; i <= cSign; i++) {
      ...
      if (!us.fSeconds &&
          (gs.nScale == 100 || gs.nFontAll == 0 || !gi.fFile ||
           gs.ft == ftBmp) &&
          y < gs.yWin - 1) {
        s = gi.nScale;
  ...
  sprintf(sz, "Car: %d,", et.coMode[0]);
  DrawPrint(sz, kModeB(0), fTrue);
  sprintf(sz, " Fix: %d,", et.coMode[1]);
  DrawPrint(sz, kModeB(1), fTrue);
  sprintf(sz, " Mut: %d", et.coMode[2]);
  DrawPrint(sz, kModeB(2), fFalse);
  ...
  sprintf(sz, "M: %d,", et.coMC);
  DrawPrint(sz, kElemB[eEar], fTrue);
  sprintf(sz, " N: %d,", et.coIC);
  DrawPrint(sz, kElemB[eWat], fTrue);
  sprintf(sz, " A: %d,", et.coAsc);
  DrawPrint(sz, kElemB[eFir], fTrue);
  sprintf(sz, " D: %d", et.coDes);
  DrawPrint(sz, kElemB[eAir], fFalse);
  sprintf(sz, "Ang: %d,", et.coModeH[0]);
  DrawPrint(sz, kModeB(0), fTrue);
  sprintf(sz, " Suc: %d,", et.coModeH[1]);
  DrawPrint(sz, kModeB(1), fTrue);
  sprintf(sz, " Cad: %d", et.coModeH[2]);
  DrawPrint(sz, kModeB(2), fFalse);
...
CONST int objNakshatra[9 + 1] = {0,    oSou, oVen, oSun, oMoo,
                                 oMar, oNod, oJup, oSat, oMer};
...
void DrawWheel(real *xsign, real *xhouse, int cx, int cy, real unitx,
  ...
  int nTrans = (int)(gs.rBackPct * 256.0 / 100.0), *rgRules, i, j, k, kSav, x,
      y, nSav;
  CONST int *rgTerm;
  ...
  if (nTrans > 0) {
    ...
    DrawCircle(cx, cy, (int)(unitx * rs2 + rRound),
               (int)(unity * rs2 + rRound));
    DrawCircle(cx, cy, (int)(unitx * rs1 + rRound),
               (int)(unity * rs1 + rRound));
  }
  ...
  if (k > 0) {
    ...
    for (i = 1; i <= cSign; i++)
      for (j = 0; j < k; j++) {
        rDeg = xhouse[i] + MinDifference(xhouse[i], xhouse[Mod12(i + 1)]) *
                               (real)j / (real)k;
        px = PX(rDeg);
        py = PY(rDeg);
...
void DrawSymbolRing(real *symbol, real *xplanet, real *obj, real *dir, int cx,
                    int cy, real unitx, real unity, real rp, real rl1, real rl2,
                    real rz, real rg) {
      ...
      if (gs.fLabel) {
        ...
        if (rz > 0.0) {
          ...
          DrawSz(sz, cx + POINT1(unitx, rz + 0.03, PX(temp)),
                 cy + POINT1(unity, rz + 0.03, PY(temp)) + gi.nScaleT * 2,
                 dtCent);
          ...
          if (chT != ' ') {
            ...
            DrawSz(sz, cx + POINT1(unitx, rz - 0.07, PX(temp)),
                   cy + POINT1(unity, rz - 0.07, PY(temp)) + gi.nScaleT * 2,
                   dtCent);
          }
        }
  ...
  for (i = 0; i <= custHi; i++) {
    ...
    if (ignore[j] || !FHasMoon(j) || j == us.objCenter || j == i)
      continue;
    if (!us.fMoonMove) {
      rx = space[i].x - space[j].x;
      ry = space[i].y - space[j].y;
    ...
    col[i] =
        (rT <= -75.0
             ? kObjB[oMC]
             : (rT >= 75.0 ? kObjB[oNad]
                           : (FBetween(temp, rDegQuad - 22.5, rDegQuad + 22.5)
                                  ? kObjB[oDes]
                                  : (FBetween(temp, 270 - 22.5, 270 + 22.5)
                                         ? kObjB[oAsc]
                                         : gi.kiGray))));
    temp += symbol[j];
    symbolM[i] = temp;
  ...
  for (i0 = 0; i0 < cHasMoons; i0++) {
    ...
    FillSymbolRingM(i, symbolM,
                    (6.0 - (real)(gs.nScale / 100)) * (real)gi.nScaleTextT2 /
                        2.0);
  }
  for (i = custHi; i >= 0; i--) {
    if (ignore[i])
    ...
    if (ignore[j] || !FHasMoon(j) || j == us.objCenter || j == i)
      continue;
    temp = symbol[j];
    x = cx + POINT1(unitx, rg, PX(temp));
    y = cy + POINT1(unity, rg, PY(temp));
    sprintf(sz, "%c", rgch[i]);
    DrawColor(col[i]);
    temp = symbolM[i];
...
void DrawRing(int iRing, int iRingMax, real xplanet[objMax],
              real symbol[objMax], int cx, int cy, real base, real ri1,
              real ri2, real rp, real rl1, real rl2, real rz, real rg,
              real rGlyph) {
  ...
  DrawSymbolRing(symbol, xplanet, pcp->obj, pcp->dir, cx, cy, unitx, unity, rp,
                 rl1, rl2, rz, rg);
  for (i = 0; i <= is.nObj; i++)
    if (!FProper(i))
      xplanet[i] = -1.0;
...
void DrawObjects(ObjDraw *rgod, int cod, int zEdge) {
  ...
  for (i = 0; i < cod; i++)
    if (rgod[i].f) {
      if (zEdge > 0 && !FInRect(rgod[i].x, rgod[i].y, zEdge, zEdge,
                                gs.xWin - zEdge, gs.yWin - zEdge)) {
        rgod[i].f = fFalse;
        continue;
      }
      obj = rgod[i].obj;
  ...
  for (i = 0; i < cod; i++)
    if (rgod[i].f) {
      ...
      for (j = 0; j < i; j++)
        if (rgod[j].f) {
          k = Min(k,
                  NAbs(rgod[i].x - rgod[j].x) + NAbs(rgod[i].yg - rgod[j].yg));
          k2 = Min(k2, NAbs(rgod[i].x - rgod[j].x) +
                           NAbs(rgod[i].yg - rgod[j].yg - zGlyph2));
        }
  ...
  for (i = cod - 1; i >= 0; i--)
    if (rgod[i].f) {
      if (zEdge > 0 && !FInRect(rgod[i].x, rgod[i].yg, zEdge, zEdge,
                                gs.xWin - zEdge, gs.yWin - zEdge))
        continue;
      obj = rgod[i].obj;
      if (rgod[i].kv != ~0) {
...
void DrawAspectLine(int obj1, int obj2, int cx, int cy, real deg1, real deg2,
                    real rx, real ry, real rz, flag fEclipse) {
  int asp = grid->n[obj1][obj2], orb = (int)(grid->v[obj1][obj2] * 3600.0), x1,
      y1, x2, y2, nDash, nSav, col;
  ...
  if (gs.nDashStyle == 1)
    nDash = NAbs(orb) / 360; // 0.1 degree steps for alpha mode
  else if ((gs.nDashMax >= 0) != gs.fAlt)
    nDash = NAbs(orb) / (60 * 60 * 2);
  else
    nDash =
        NAbs(orb) * NAbs(gs.nDashMax) / (int)(GetOrb(obj1, obj2, asp) * 3600.0);
  DrawAspectDash(x1, y1, x2, y2, nDash);
  ...
  if (gs.fLabelAsp) {
    ...
    gi.nScale = gi.nScaleTextT;
    DrawAspect(
        asp + (fEclipse && NCheckEclipseAny(obj1, asp, obj2, NULL) > etNone) *
                  cAspect2,
        (x1 + x2) >> 1, (y1 + y2) >> 1);
    gi.nScale = nSav;
  }
...
LNext:
  ...
  if (!*pch && nC <= 0) {
    ...
    lon = nDegMax - (((pch[2] - '0') * 10 + (pch[3] - '0')) * 15 +
                     (pch[4] - '0') * 10 + (pch[5] - '0'));
    lat = 90 -
          ((pch[6] == '-' ? -1 : 1) * ((pch[7] - '0') * 10 + (pch[8] - '0')));
    if (lon >= nDegMax)
      lon -= nDegMax;
    pch += 9;
    xLo = xHi = xold = x = lon;
  ...
  loop {
    if (nC <= 0) {
      ...
      case 'R':
        ...
        break; // Right until
      default:
        PrintError("Bad constel enum."); // Shouldn't happen.
      }
      if (chCmd >= 'a')
...
int LookupConstel(real lon, real lat) {
  ...
  if (gi.rgConstel == NULL) {
    ...
    for (iConst = 1; iConst <= cCnstl; iConst++) {
      iTop = 1;
      iCur = 0;
      x = rgptConstel[iConst].x;
      y = rgptConstel[iConst].y;
      RgCon(x, y) = iConst;
      rgpt[0].x = x;
      rgpt[0].y = y;
  ...
  if (fGlobe) {
    ...
    if (gs.fMollewide)
      j = (int)(180.0 * gi.rScaleX) +
          (int)((x1 - 180.0) * RMollewide(y1 - 90.0) / 180.0 * gi.rScaleX +
                rRound);
  }
  *xp = j;
  *yp = k;
...
void DrawMapLine(real lon1, real lat1, real lon2, real lat2, flag fGlobe,
                 int nScl, CIRC *pcr, real deg) {
  int x1, y1, x2, y2;
  real lon3, lat3;
#ifdef WIRE
  int z1, z2;
  ...
  if (!FMapCalc(lon1, lat1, &x1, &y1, fGlobe, -1, 0.0, nScl, pcr, deg) &&
    ...
    if (gi.nMode != gWorldMap || gs.fMollewide) {
      ...
      if (lat1 <= 0.0 || lat1 >= rDegHalf)
        ...
        x2 = x1;
      DrawWrap(x1, Min(y1, gs.yWin - 1), x2, Min(y2, gs.yWin - 1), 0,
               gs.xWin - 1);
    }
  }
}
...
void DrawMapTriangle(real lon1, real lat1, real lon2, real lat2, real lon3,
                     real lat3, flag fGlobe, int nScl, CIRC *pcr, real deg,
                     int nLevel, int grf) {
  ...
  if (nLevel > 0) {
    ...
    DrawMapTriangle(x1, y1, x2, y2, x3, y3, fGlobe, nScl, pcr, deg, nLevel - 1,
                    7);
...
void DrawMapSquare(real lon1, real lat1, real lon2, real lat2, real lon3,
                   real lat3, real lon4, real lat4, flag fGlobe, int nScl,
                   CIRC *pcr, real deg, int nLevel, int grf) {
...
void DrawMapTriangles(flag fGlobe, int nScl, CIRC *pcr, real deg) {
  ...
  if (gs.nTriangles > 0) {
    ...
    for (x1 = 0.0; x1 < rDegMax; x1 += 72.0) {
      DrawMapTriangle(x1, y1, x1 + 72.0, y1, x1 + 36.0, 90.0, fGlobe, nScl, pcr,
                      deg, i, 5);
      DrawMapTriangle(x1, y1, x1 + 72.0, y1, x1 + 36.0, -y1, fGlobe, nScl, pcr,
                      deg, i, 4);
      DrawMapTriangle(x1 + 36.0, -y1, x1 + 108.0, -y1, x1 + 72, -90.0, fGlobe,
                      nScl, pcr, deg, i, 5);
      DrawMapTriangle(x1 + 36.0, -y1, x1 + 108.0, -y1, x1 + 72, y1, fGlobe,
                      nScl, pcr, deg, i, 4);
    }
    ...
    for (x1 = 0.0; x1 < rDegMax; x1 += rDegQuad)
      DrawMapSquare(x1, y1, x1, -y1, x1 + 90.0, -y1, x1 + 90.0, y1, fGlobe,
                    nScl, pcr, deg, i, 11);
    for (x1 = -y1; x1 <= y1; x1 += y1 * 2.0)
      DrawMapSquare(0.0, x1, 90.0, x1, 180.0, x1, 270.0, x1, fGlobe, nScl, pcr,
                    deg, i, 0);
  }
}
...
void DrawMap(flag fSky, flag fGlobe, real deg) {
  ...
  while (
    ...
    if (fSky) {
      ...
      if (i >= 0)
        DrawColor((!fGlobe && gi.nMode == gAstroGraph)
                      ? gi.kiOn
                      : (!gs.fAlt && !gs.fColorHouse
                             ? gi.kiGray
...
LAfter:
  ...
  for (j = 0; j < arMax; j++) {
    ...
    for (i = 0; i <= is.nObj; i++)
      if (FProper(i)) {
        ...
        l = j * objMax + i;
        rgod[l].f = !FMapCalc(x1, y1, &rgod[l].x, &rgod[l].y, fGlobe, fSky, rT,
                              nScl, &cr, deg);
        rgod[l].obj = i;
  ...
  for (i = is.nObj; i >= 0; i--)
    if (FProper(i)) {
      ...
      for (k = 1; k <= cSign; k++) {
        if (FIgnore(cuspLo - 1 + k) || (k == sAri && chouse[k] == is.Asc) ||
  ...
  if (us.fLatitudeCross &&
      !FMapCalc(Mod(rDegHalf - Lon + (!fGlobe ? deg : 0.0)), rDegQuad - Lat, &j,
                &k, fGlobe, -1, 0.0, nScl, &cr, deg)) {
    DrawColor(kMagentaB);
    DrawSpot(j, k);
  }
...
flag EnumStarsLines(flag fInit, ES **ppes1, ES **ppes2) {
  ...
  if (gi.rges == NULL || !FBetween(i1, 0, gi.cStarsLin - 1) ||
      !FBetween(i2, 0, gi.cStarsLin - 1))
    return fFalse;
  *ppes1 = &gi.rges[i1];
  *ppes2 = &gi.rges[i2];
  return fTrue;
...
void DrawChartX() {
  ...
  if (fDrawText) {
    if (FNoTimeOrSpace(ciMain))
      ...
      fSav = us.fAnsiChar;
      us.fAnsiChar = (gs.nFontTxt == 0 || (gs.ft != ftPS && gs.ft != ftWmf))
                     << 1;
      i = DayOfWeek(Mon, Day, Yea);
      sprintf(sz, "%s%s%.3s %s %s (%s) %s%s%s",
              FSzSet(ciCore.nam) ? ciCore.nam : "",
              FSzSet(ciCore.nam) ? ", " : "", szDay[i],
              SzDate(Mon, Day, Yea, 2), SzTim(Tim), SzOffset(Zon, Dst, Lon),
              FSzSet(ciCore.loc) ? ciCore.loc : "",
              FSzSet(ciCore.loc) ? " " : "", SzLocation(Lon, Lat));
      us.fAnsiChar = fSav;
    }
    i = dtBottom | dtScale2;

    if (!(gi.fBmp && gi.bmpBack.rgb != NULL) || gi.nMode == gGrid ||
        gi.nMode == gLocal || gi.nMode == gTraTraGra ||
